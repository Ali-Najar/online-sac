...
def scan(f, init, xs, out, checkpoint_group=0):
    ...
    if checkpoint_group > 0 and num_items > checkpoint_group:
    ...
    def forward(
        ...
        if attention_mask is None:
            if input_ids is not None:
                ...
                # inputs_embeds must be provided; create a mask of ones with shape (batch, seq_len)
                attention_mask = torch.ones(
                    inputs_embeds.shape[:-1],  # (batch, seq_len)
                    dtype=torch.long,
                    device=inputs_embeds.device
                )
        # if attention_mask is None:
